...
Output format
Return only a Markdown document with this structure, saved in the `project-documentation/spec-to-plan.md`:
...
*In a team of N engineers, the scoped plan is feasible within ≤ 6 months assuming average velocity of X points/sprint and parallelization across phases with clear interfaces.*  
*Critical path items are {list top bottlenecks}; schedule risk is moderated via flags/canary + early CI/CD and test automation.*
