...
CREATE UNIQUE INDEX IF NOT EXISTS idx_care_plans_client_unique ON care_plans(client_id) WHERE deleted_at IS NULL;

-- Fast lookup by client for care plan retrieval
-- Most common query: "Get care plan for client X"
...
CREATE INDEX IF NOT EXISTS idx_care_plans_medications ON care_plans USING GIN (medications) WHERE deleted_at IS NULL;

-- Support for allergy searches
-- Query pattern: "Find all clients with allergy X"
-- GIN index enables efficient JSONB array queries
CREATE INDEX IF NOT EXISTS idx_care_plans_allergies ON care_plans USING GIN (allergies) WHERE deleted_at IS NULL;

-- Version tracking for conflict detection
-- Query pattern: "Check if care plan has been updated since last read"
CREATE INDEX IF NOT EXISTS idx_care_plans_version ON care_plans(client_id, version) WHERE deleted_at IS NULL;
...
COMMENT ON TABLE care_plans IS 'Care plan information for clients including medications, allergies, and special instructions';
