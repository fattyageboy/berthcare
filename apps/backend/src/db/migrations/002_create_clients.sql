...
CREATE INDEX IF NOT EXISTS idx_clients_zone_id ON clients(zone_id) WHERE deleted_at IS NULL;

-- Fast lookup by last name for search functionality
-- Common query: "Search clients by last name"
CREATE INDEX IF NOT EXISTS idx_clients_last_name ON clients(last_name) WHERE deleted_at IS NULL;

-- Composite index for zone + last name queries (common in client lists)
-- Optimizes: "Get all clients in zone X, sorted by last name"
CREATE INDEX IF NOT EXISTS idx_clients_zone_last_name ON clients(zone_id, last_name) WHERE deleted_at IS NULL;

-- Geographic queries for route optimization
-- Supports: "Find clients near this location"
CREATE INDEX IF NOT EXISTS idx_clients_location ON clients(latitude, longitude) WHERE deleted_at IS NULL;

-- Full name search support (case-insensitive)
-- Supports: "Search clients by first or last name"
CREATE INDEX IF NOT EXISTS idx_clients_full_name ON clients(LOWER(first_name), LOWER(last_name)) WHERE deleted_at IS NULL;
...
COMMENT ON TABLE clients IS 'Client (patient) information for home care services';
